...
function MetaHudO_Header_OnLoad(this,x,y)
...
function MetaHudO_DropDown_OnClick(this, list)
     list = tmp;
     local sel = _G["MetaHud_Edit_"..list.."_Selection"];
     local box = _G["MetaHud_Edit_"..list.."ScrollFrameText"];
...
function MetaHud_ProfilesDropDown_OnClick(this)
...
function MetaHud_SwitchButton_Select(index)
     local MAX_SELECT = 14;
     if(index == "All") then
          local set = {"layouttyp", "texturefile", "fontfile"};
          for x=1, 3, 1 do
               for i=1, MAX_SELECT, 1 do
                    local switch = _G["MetaHud_"..set[x]..i];
                    if(switch ~= nil) then
          ...
          for i=1, MAX_SELECT, 1 do
               local switch = _G["MetaHud_"..index..i];
               if(switch ~= nil) then
...
function MetaHud_SwitchButton_OnClick(self, button)
     local index = string.gsub( self:GetName() , "MetaHud_", "");
     index = string.gsub( index , self:GetID(), "");
     MetaHud:SetConfig(index, self:GetID());
     MetaHud_SwitchButton_Select(index);
end

function MetaHudO_updateTexts(name)
     MetaHud:initTextfield(_G[name],name);
     MetaHud:doText(name);
end

function MetaHudO_FrameSlider_OnLoad(this, low, high, step)
     this.setting_name = string.gsub( this:GetName() , "MetaHud_Slider_", "");
     this.st = step;
     if this.nullbase == 1 then
          _G[this:GetName().."Low"]:SetText( (low - low) );
          _G[this:GetName().."High"]:SetText( (high - low));     
          this.low = low;  
     else
          _G[this:GetName().."Low"]:SetText(low);
          _G[this:GetName().."High"]:SetText(high);        
     end
     this:SetMinMaxValues(low, high);
     this:SetValueStep(this.st);
end

function MetaHudO_FrameSlider_OnShow(this,key,text)
     this:SetValue(MetaHudOptions[key]);
     if this.nullbase == 1 then
          _G[this:GetName().."Text"]:SetText(text.." |cff00ff00"..(MetaHudOptions[key] - this.low ).."|r ");
     else
          _G[this:GetName().."Text"]:SetText(text.." |cff00ff00"..MetaHudOptions[key].."|r ");
     end
end

function MetaHudO_FrameSlider_OnValueChanged(this,key,text)
     local m;
     local value;
...
function MetaHudO_ColorPicker_OnClick(this, boxnumber)
...
function MetaHudO_changeG(name)
     ...
     _G[basename.."1Texture"]:SetTexture(c1r,c1g,c1b);
     _G[basename.."2Texture"]:SetTexture(c2r,c2g,c2b);
     _G[basename.."3Texture"]:SetTexture(c3r,c3g,c3b);
     _G[basename.."G1Texture"]:SetGradient("HORIZONTAL",c1r,c1g,c1b, c2r,c2g,c2b);
     _G[basename.."G2Texture"]:SetGradient("HORIZONTAL",c2r,c2g,c2b, c3r,c3g,c3b);
end
